...
A single HTML file (`shotlist.html`) saved to `outputs/` and presented to the user. Structure:
...
7. **Save to `outputs/shotlist.html`** and present it.
...
Read the previous shotlist if it's still in context or in `outputs/`, apply the user's edits, and write the updated file back. Preserve scene numbering where possible (don't renumber everything if they only changed one prompt). Preserve the Style Prefix unless they tell you to change it.
